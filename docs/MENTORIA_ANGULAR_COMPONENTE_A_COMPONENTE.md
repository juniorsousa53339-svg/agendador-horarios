...
## 0.1) Como criar componentes na prática (passo a passo de mentoria)

Se você travar em qualquer tela, usa **sempre este roteiro**:
...
### Passo 2 — Definir a responsabilidade do componente (1 frase)
Antes de codar, escreva:
- **O que esse componente faz?**
- **Quais dados ele recebe?**
- **Quais ações ele dispara?**

Exemplo (`AgendarComponent`):
- Faz cadastro de agendamento público;
- Recebe lista de funcionários e serviços (via API);
- Dispara ação de `confirmar()`.

### Passo 3 — Montar o TypeScript primeiro (`.ts`)
Regra simples:
1. Criar propriedades (listas/estado);
2. Criar `FormGroup` (se tiver formulário);
3. Injetar serviços no `constructor`;
4. Implementar métodos (`carregar`, `salvar`, `confirmar`, etc.).

Checklist rápido do `.ts`:
- [ ] `standalone: true`;
- [ ] `imports` corretos (`ReactiveFormsModule`, `CommonModule` quando precisar de `*ngFor`);
- [ ] Tipagem dos arrays (`Funcionario[]`, `Servico[]`, etc.);
- [ ] Método de submit validando `form.invalid`.

### Passo 4 — Depois montar o HTML (`.html`)
No template, siga esta ordem:
1. título da tela;
2. formulário/filtros;
3. botão de ação;
4. listagem de resultado.

Checklist rápido do `.html`:
- [ ] `formGroup` e `formControlName` batendo com o `.ts`;
- [ ] `*ngFor` apenas se tiver lista carregada;
- [ ] placeholders claros para quem usa.

### Passo 5 — Estilizar o básico (`.scss`)
Comece com o mínimo:
- container;
- spacing (`margin`/`padding`);
- botão principal e campos.

> Não tente “deixar bonito perfeito” antes de funcionar.

### Passo 6 — Conectar rota no `app.routes.ts`
Sempre que criar tela nova, adicione rota com `loadComponent`.

### Passo 7 — Testar isolado
Teste em ciclos curtos:
1. componente renderiza;
2. formulário valida;
3. chamada API funciona;
4. feedback de sucesso/erro aparece.

### Passo 8 — Só depois reutilizar em outros lugares
Quando copiar padrão para outro componente, duplique a estrutura e só troque:
- nome da entidade (`Funcionario` → `Servico`);
- service (`FuncionarioApiService` → `ServicoApiService`);
...
## 1.3 Roadmap guiado no VS Code (do jeito que você descreveu)

> **Situação comum:** você criou só as pastas (`pages/cliente`, `pages/funcionario`, etc.) e elas ficaram “vazias”, sem `.html`, `.ts` e `.scss`.
>
> **Resposta curta:** sim, isso é normal quando usa `mkdir`.  
> Para criar componente Angular completo, use **Angular CLI (`ng g c`)**, não só criação manual de pasta.

### Fluxo exato (copiar e colar no terminal)

1) Entrar na raiz do projeto Angular (onde está `angular.json`):
```bash
cd agendador-front
```

2) Criar a estrutura base de pastas (opcional):
```bash
mkdir -p src/app/core/{services,guards,interceptors}
mkdir -p src/app/shared/{models,components}
mkdir -p src/app/pages/{auth,cliente,funcionario,proprietario}
```

3) Gerar os componentes de páginas (aqui surgem `.ts`, `.html`, `.scss`):
```bash
ng g c pages/auth/login --standalone
ng g c pages/cliente/agendar --standalone
ng g c pages/funcionario/agenda-funcionario --standalone
...
4) Gerar componentes compartilhados:
```bash
ng g c shared/components/card --standalone
ng g c shared/components/page-header --standalone
ng g c shared/components/loading --standalone
ng g c shared/components/sidebar --standalone
ng g c layout/shell --standalone
```

5) Conferir no Explorer do VS Code se os arquivos foram criados:
- cada componente precisa ter, no mínimo:
  - `nome.component.ts`
  - `nome.component.html`
  - `nome.component.scss`

6) Só depois configurar as rotas (`app.routes.ts`) apontando para cada componente.

### Como saber que deu certo?
- Se você expandir `src/app/pages/cliente/agendar`, deve ver os 3 arquivos do componente.
- Se só aparecer pasta vazia, o componente **não foi gerado** (só a pasta foi criada).

### Erro que apareceu no seu terminal (igual ao print)
Você tentou mover arquivo que não existe (`src/app/cliente/arquivo.ts`).  
Faça assim:
1. gere o componente com `ng g c ...`;
2. depois mova/edite somente arquivos reais que o Angular criou.
...
### Exemplo de uso dos componentes compartilhados
```html
<app-page-header
  titulo="Gestão de Serviços"
  subtitulo="Cadastre e mantenha os serviços da barbearia">
</app-page-header>

<app-card titulo="Novo serviço">
  <!-- formulário aqui -->
</app-card>
...
5. **Criei pastas em `pages`, mas não tenho `.html/.scss/.ts`**
   - Isso acontece quando usa `mkdir` manual.
   - Gere com Angular CLI: `ng g c pages/cliente/agendar --standalone`.
